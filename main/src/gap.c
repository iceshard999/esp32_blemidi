...
static void start_advertising(void) {
    ...
    ESP_LOGI(TAG, "Starting advertising configuration...");

    // BLE MIDI服务的128-bit UUID（小端格式）
    ...
    adv_fields.uuids128 = &midi_service_uuid;
    ...
    adv_fields.name = (uint8_t *)"MIDI";
    adv_fields.name_len = strlen("MIDI");
    ...
    ESP_LOGI(TAG, "Advertisement fields set successfully");
    ...
    /* Set connectable and general discoverable mode for GATT server */
    adv_params.conn_mode = BLE_GAP_CONN_MODE_UND;
    adv_params.disc_mode = BLE_GAP_DISC_MODE_GEN;

    /* Optional parameters */
    adv_params.filter_policy = 0;  // No white list
    adv_params.high_duty_cycle = 0;  // Normal advertising duty cycle

    /* Set advertising interval */
    adv_params.itvl_min = BLE_GAP_ADV_ITVL_MS(500);
    adv_params.itvl_max = BLE_GAP_ADV_ITVL_MS(510);

    /* Try to stop advertising first (ignore error if not advertising) */
    ble_gap_adv_stop();
    ESP_LOGI(TAG, "Attempting to start advertising...");
...
static int gap_event_handler(struct ble_gap_event *event, void *arg) {
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            struct ble_gap_upd_params params = {
                ...
                .latency = 0,     // 无延迟
                .supervision_timeout = 400, // 5s (400 * 10ms)
            };
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        ...
        switch (event->subscribe.prev_notify)
        {
            case 0:
                ...
                break;
            case 1:
                /* GATT unsubscribe event callback */
                gatt_svr_subscribe_cb(event);
                break;
            default:
                break;
